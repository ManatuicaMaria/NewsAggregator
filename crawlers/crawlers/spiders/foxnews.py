...
class FoxNewsSpider(SitemapSpider):
    ...
    def parse(self, response):
        item_data = {
            ...
            "date": parse(response.xpath('//meta[@name="dc.date"]/@content').extract()[0], fuzzy=True).date(),
            "description": remove_unicode(response.xpath('//meta[@name="dc.description"]/@content').extract()[0].strip()),
            "content": remove_unicode(' '.join(response.xpath('//*[@class="article-body"]/p//text()').extract()).strip()),
            "url": response.url,
        }
